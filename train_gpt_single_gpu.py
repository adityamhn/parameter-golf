...
def _sdpa_attn(q, k, v, causal=True):
    """Drop-in replacement for flash_attn_func using PyTorch SDPA.
    Inputs/outputs use [B, T, H, D] layout (same as flash_attn)."""
    y = F.scaled_dot_product_attention(
        q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2),
        is_causal=causal, enable_gqa=True,
    )
    return y.transpose(1, 2)
flash_attn_3_func = _sdpa_attn
class Hyperparameters:
    ...
    warmup_steps = int(os.environ.get("WARMUP_STEPS", 15))
    ...
    muon_momentum_warmup_start = float(os.environ.get("MUON_MOMENTUM_WARMUP_START", 0.89))
    ...
    leaky_slope = float(os.environ.get("LEAKY_SLOPE", 0.72))
    ...
    export_artifacts = bool(int(os.environ.get("EXPORT_ARTIFACTS", "1")))
    post_ema_diagnostic = bool(int(os.environ.get("POST_EMA_DIAGNOSTIC", "0")))
    final_int6_eval = bool(int(os.environ.get("FINAL_INT6_EVAL", "0")))
    force_stride64_eval = bool(int(os.environ.get("FORCE_STRIDE64_EVAL", "0")))
...
def main() -> None:
    ...
    grad_accum_steps = int(os.environ.get("GRAD_ACCUM_STEPS", str(8 // max(world_size, 1))))
    ...
    if args.export_artifacts and master_process:
    ...
    if args.export_artifacts:
        ...
        if distributed:
            dist.barrier()
        if args.final_int6_eval or args.force_stride64_eval or args.ttt_enabled or (args.eval_stride > 0 and args.eval_stride < sw_seq_len):
    ...
    if args.final_int6_eval and eval_model is not None:
        torch.cuda.synchronize()
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len and eval_model is not None:
        torch.cuda.synchronize()
    ...
    if args.force_stride64_eval and args.eval_stride != 64 and 64 < sw_seq_len and eval_model is not None:
        torch.cuda.synchronize()
